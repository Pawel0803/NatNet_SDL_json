...
class NatNetClient:
    ...
    def set_all_jsondata(self, jsondata):
        self.alljsondata = jsondata
...
# Unpack Mocap Data Functions
    ...
    def __unpack_labeled_marker_data(self, data, packet_size, major, minor):
        ...
        print(f"Labeled Marker Data - Długość: {len(data)}, Packet Size: {packet_size}")

        # Sprawdź, czy mamy wystarczająco danych na początek
        if len(data) < 4:
            print("Ostrzeżenie: <LOCATION> ilość danych dla labeled markers")
        ...
        if ((major == 2 and minor > 3) or major > 2):
            ...
            # Sprawdź, czy liczba markerów jest sensowna
            if labeled_marker_count < 0 or labeled_marker_count > 1000:  # Arbitralny limit
                print(f"Ostrzeżenie: Podejrzana liczba markerów: {labeled_marker_count}")
                return offset, labeled_marker_data

            # Sprawdź, czy mamy dane dla data size
            if offset + 4 > len(data):
                print(f"Ostrzeżenie: <LOCATION> ilość danych dla data size")
                return offset, labeled_marker_data
            ...
            try:
                ...
                offset += offset_tmp
                print(f"Unpacked Data Size: {unpackedDataSize}")
            except Exception as e:
                print(f"Błąd podczas odczytu data size: {e}")
                return offset, labeled_marker_data

            # Iteruj przez markery z bezpiecznym sprawdzaniem
            for lm_num in range(0, labeled_marker_count):
                # Sprawdź, czy mamy dane dla ID markera
                if offset + 4 > len(data):
                    print(
                        f"Ostrzeżenie: <LOCATION> ilość danych dla ID <LOCATION> {lm_num} (offset={offset}, len={len(data)})")
                    break
                ...
                try:
                    ...
                    print(f"Błąd podczas odczytu ID markera {lm_num}: {e}")
                    break

                # Sprawdź, czy mamy dane dla pozycji markera
                if offset + 12 > len(data):
                    print(
                        f"Ostrzeżenie: <LOCATION> ilość danych dla pozycji markera {lm_num} (offset={offset}, len={len(data)})")
                    break
                ...
                try:
                    ...
                    print(f"Błąd podczas odczytu pozycji markera {lm_num}: {e}")
                    break

                # Sprawdź, czy mamy dane dla rozmiaru markera
                if offset + 4 > len(data):
                    print(
                        f"Ostrzeżenie: <LOCATION> ilość danych dla rozmiaru markera {lm_num} (offset={offset}, len={len(data)})")
                    print(f"Dostępne bajty: {len(data) - offset}")

                    # Wyświetl dostępne bajty
                    if len(data) - offset > 0:
                        hex_data = ' '.join(f'{b:02x}' for b in bytes(data[offset:]))
                        print(f"Pozostałe bajty: {hex_data}")

                    # Użyj domyślnego rozmiaru i kontynuuj
                    size = 0.0
                    labeled_marker = MoCapData.LabeledMarker(tmp_id, pos, size, 0, 0.0)
                    labeled_marker_data.add_labeled_marker(labeled_marker)
                    offset = len(data)  # Ustaw offset na koniec danych
                    break

                # Wyświetl bajty jako hex przed odczytem
                hex_data = ' '.join(f'{b:02x}' for b in bytes(data[offset:offset + 4]))
                print(f"Bajty: {hex_data}")

                try:
                    size = FloatValue.unpack(data[offset:offset + 4])
                    offset += 4
                except struct.error as e:
                    print(f"Błąd przy odczycie rozmiaru markera {lm_num}: {e}")
                    size = 0.0
                    offset = len(data)  # Ustaw offset na koniec danych
                    break

                # Version 2.6 and later - parametr
                param = 0
                if ((major == 2 and minor >= 6) or major > 2):
                    # Sprawdź, czy mamy dane dla parametru
                    if offset + 2 > len(data):
                        print(f"Ostrzeżenie: <LOCATION> ilość danych dla parametru markera {lm_num}")
                        labeled_marker = MoCapData.LabeledMarker(tmp_id, pos, size, param, 0.0)
                        labeled_marker_data.add_labeled_marker(labeled_marker)
                        offset = len(data)  # Ustaw offset na koniec danych
                        break
                    ...
                    try:
                        ...
                        print(f"Błąd przy odczycie parametru markera {lm_num}: {e}")
                        offset = len(data)  # Ustaw offset na koniec danych
                        break

                # Version 3.0 and later - residual
                residual = 0.0
                if major >= 3:
                    # Sprawdź, czy mamy dane dla residual
                    if offset + 4 > len(data):
                        print(f"Ostrzeżenie: <LOCATION> ilość danych dla residual <LOCATION> {lm_num}")
                    ...
                    try:
                        ...
                        print(f"Błąd przy odczycie residual markera {lm_num}: {e}")
                        offset = len(data)  # Ustaw offset na koniec danych
                        break

                # Dodaj marker do danych
                labeled_marker = MoCapData.LabeledMarker(tmp_id, pos, size, param, residual)
                labeled_marker_data.add_labeled_marker(labeled_marker)
